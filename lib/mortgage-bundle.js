...
var MortgageBundle = (() => {
  ...
  function applyFallbacks(row, domain) {
    const r = { ...row };
    for (const sub of domain.subscribes) {
      if (r[sub.name] !== void 0) continue;
      const fb = domain.subscriptionFallbacks[sub.name];
      if (!fb) continue;
      const source = r[fb.fromParam];
      if (source === void 0) continue;
      r[sub.name] = fb.transform ? fb.transform(source) : source;
    }
    return r;
  }
  function validateDomain(domain) {
    const paramNames = new Set(domain.parameters.map((p) => p.name));
    for (const sub of domain.subscribes) {
      const fb = domain.subscriptionFallbacks[sub.name];
      if (!fb) {
        throw new Error(
          `Domain ${domain.name} subscribes to ${sub.name} but declares no fallback. Every subscription MUST have a fallback (ADR-003 \xA7Decision 3).`
        );
      }
      if (!paramNames.has(fb.fromParam)) {
        throw new Error(
          `Domain ${domain.name} subscription ${sub.name} fallback references unknown parameter ${fb.fromParam}. Must be a real parameter in domain.parameters.`
        );
      }
    }
  }
...
IF [credit_tier] = "Subprime" AND [loan_type] = "FHA" THEN [down_pct] >= 10;`
  ...
  var mortgageDomain = {
    ...
    // Fallbacks resolved by the framework's applyFallbacks pass (ADR-003 §3)
    // before derivations run. The transform on monthly_take_home does the
    // annual→monthly conversion that the dti derivation used to do inline.
    // The dti derivation still carries a backstop branch that handles the
    // pre-applyFallbacks shape — kept until sc-mortgage-demo is updated to
    // call applyFallbacks (it consumes the bundle directly today).
    subscriptionFallbacks: {
      monthly_take_home: {
        fromParam: "annual_income_fallback",
        transform: (v) => Number(v) / 12
      },
      monthly_existing_debts: {
        fromParam: "monthly_debts_fallback"
      }
    }
  };
  var seedScenarios = [
  ...
  function buildMortgageScenarios() {
    validateDomain(mortgageDomain);
    return seedScenarios.map((row, i) => {
      const withFallbacks = applyFallbacks(row, mortgageDomain);
      const withBuckets = applyVirtualParams(withFallbacks, mortgageDomain.virtualParameters ?? []);
