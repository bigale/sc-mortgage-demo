...
//----------------------------------------
    var currentSkin = isc.setCurrentSkin({
        ...
        series: "Flat",
        // base font-list
        fonts: ["Roboto-Light", "corbel", "calibri"]
    });
...
//----------------------------------------
    ...
    isc.Canvas.registerIconSizingAttributes(
        "fonts",
        {
            ...
            // HeaderItem height needs to scale because it is closely sized to its font, which 
            // also scales and can end up being clipped - HeaderItem is floating, top-aligned 
            // text with some padding and no border, so the full sizeIncrease adds too much 
            // v-space beneath - instead, scale it by fontIncrease to avoid text-clipping 
            // without adding too much blank v-space
            HeaderItem: [ "height" ]
        }
    );
    ...
    if (isc.DateRangeDialog) {
        isc.DateRangeDialog.changeDefaults("headerIconProperties", { src:"[SKIN]/pickers/date_picker.png" });
    ...
    if (isc.TabSet) {
        ...
        isc.TabSet.changeDefaults("tabBarControlLayoutDefaults", { 
            styleName: "tabControlLayout",
            padding: 0
        });
    ...
    if (isc.Menu) {
        ...
        isc.Menu.changeDefaults("keyFieldDefaults", {
            baseStyle: "menuKeyField"
        });
    ...
    if (isc.ListGrid) {
        isc.ListGrid.addProperties({
            ...
            booleanTrueImage: "sprite:cssClass:checkboxTrue;size:38,38",
            booleanFalseImage: "sprite:cssClass:checkboxFalse;size:38,38",
            booleanPartialImage: "sprite:cssClass:checkboxPartial;size:38,38",
            ...
            checkboxFieldImageHeight:16,
            checkboxFieldImageWidth:16,
            ...
            spannedHeaderMenuBaseStyle: "headerMenuButton",
            headerHeight:28,//38
            ...
            , sortArrowMenuButtonSpaceOffset: 10
        });
    ...
    if (isc.DynamicForm) {
        ...
        // with the labelAnchor style, ensuring that all three options static, readOnly 
        // and disabled for readOnlyDisplay use the same consistent style.
        isc.DynamicForm.addProperties({ 
            readOnlyTextBoxStyle: "labelAnchor",
            styleName: "form"
        });
    }
    ...
    if (isc.CheckboxItem) {
        isc.CheckboxItem.addProperties({
            ...
            checkedImage: "sprite:cssClass:checkboxTrue;size:38,38",
            uncheckedImage: "sprite:cssClass:checkboxFalse;size:38,38",
            // For this theme there is no default "unset" image
            unsetImage: "sprite:cssClass:checkboxPartial;size:38,38",
            partialSelectedImage: "sprite:cssClass:checkboxPartial;size:38,38",
    ...
    if (isc.ComboBoxItem) {
        isc.ComboBoxItem.addProperties({
            pickerIconSrc:"sprite:cssClass:comboBoxItemPicker;size:24,24;offset:5,8;",
            pickerIconHeight:16,
            pickerIconWidth:16,
    ...
    if (isc.RelativeDateItem) {
        isc.RelativeDateItem.addProperties({
            ...
            pickerIconSrc: "[SKIN]/pickers/date_picker.png"
        });
    ...
    if (isc.DateGrid) {
        isc.DateGrid.addProperties({
            headerBarStyle: "dateGridHeaderBar",
            minFieldWidth:33,
            cellHeight:27,
            alternateFieldStyles: false,
    ...
    if (isc.Calendar) {
        isc.Calendar.changeDefaults("datePickerButtonDefaults", {
            showDown:false,
            showOver:false,
            src:"[SKIN]/pickers/date_picker.png"
        });
    ...
    if (isc.MiniNavControl) {
        isc.MiniNavControl.addProperties({
            src: isc.Browser.isIPhone ? "[SKIN]/miniNav.svg" : "[SKIN]/miniNav.png",
            showDisabled: true,
            upButtonSrc: null,
            downButtonSrc: null
        });
